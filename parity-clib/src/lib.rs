...
pub unsafe extern fn parity_config_from_cli(args: *const *const c_char, args_lens: *const usize, len: usize, output: *mut *mut c_void) -> c_int {
	panic::catch_unwind(|| {
		*output = ptr::null_mut();
		...
		match reosc::Configuration::parse_cli(&args) {
			Ok(mut cfg) => {
...
pub unsafe extern fn parity_config_destroy(cfg: *mut c_void) {
	let _ = panic::catch_unwind(|| {
		let _cfg = Box::from_raw(cfg as *mut reosc::Configuration);
	});
}
...
pub unsafe extern fn parity_start(cfg: *const ParityParams, output: *mut *mut c_void) -> c_int {
	panic::catch_unwind(|| {
		*output = ptr::null_mut();
		...
		let config = Box::from_raw(cfg.configuration as *mut reosc::Configuration);
		...
		let action = match reosc::start(*config, on_client_restart_cb, || {}) {
			Ok(action) => action,
			Err(_) => return 1,
		};

		match action {
			reosc::ExecutionAction::Instant(Some(s)) => { println!("{}", s); 0 },
			reosc::ExecutionAction::Instant(None) => 0,
			reosc::ExecutionAction::Running(client) => {
				*output = Box::into_raw(Box::<reosc::RunningClient>::new(client)) as *mut c_void;
				0
			}
		}
	}).unwrap_or(1)
...
pub unsafe extern fn parity_destroy(client: *mut c_void) {
	let _ = panic::catch_unwind(|| {
		let client = Box::from_raw(client as *mut reosc::RunningClient);
		client.shutdown();
	});
}
...
pub unsafe extern fn parity_rpc(client: *mut c_void, query: *const c_char, len: usize, out_str: *mut c_char, out_len: *mut usize) -> c_int {
	panic::catch_unwind(|| {
		let client: &mut reosc::RunningClient = &mut *(client as *mut reosc::RunningClient);
