...
trace!(target: "spec", "BlockNum {:?}", number);

				let mut result_block_reward = U256::from_dec_str("250000000000000000000").unwrap();
				let mut ubi_reward          = U256::from_dec_str("50000000000000000000").unwrap();
...
//					dev_contract = "0xc0471f97dd28c6a375f32f604f9c04ed17cf961d".into();
				...
				if number >= 64000 { 
					...
					// fixing uncle rewards error in start - it was no uncle reward before this block
					// and we paid for uncle rewards from contract wallet
