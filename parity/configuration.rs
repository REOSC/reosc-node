...
impl Configuration {
	...
	///
	/// ```
	/// let _cfg = reosc::Configuration::parse_cli(&["--light", "--chain", "kovan"]).unwrap();
	/// ```
	...
	fn net_config(&self) -> Result<NetworkConfiguration, String> {
		...
		ret.client_version = {
			...
			if !self.args.arg_identity.is_empty() {
				// Insert name after the "REOSC/" at the beginning of version string.
	...
	fn test_identity_arg() {
		...
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				...
				assert!(c.net_conf.client_version.starts_with("REOSC/Somebody/"));
			}
			_ => panic!("Should be Cmd::Run"),
		}
	}
